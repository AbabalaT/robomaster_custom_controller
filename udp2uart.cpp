...
int main() {
    ...
    int recv_num = 0;
    auto ttyUSB = uart_start("/dev/ttyUSB0");
    ...
    uint8_t test_word[64]={0xa5, 0x0c, 0x00, 0x00, 0x78, 0x04, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,0x00, 0x00, 0x00, 0x00, 0x00, 0x00,0x00, 0x00};
    //cout<<Get_CRC16_Check_Sum(test_word, 19, 0xFFFF)<<endl;
    //cout<<test_word[19]<<"  "<<test_word[20]<<endl;
    while (1) {
        recv_num = recvfrom(sockfd, (char*) recv_buf, 64, 0, (struct sockaddr *)&addr, &addr_len);
        recv_buf[4] = Get_CRC8_Check_Sum(recv_buf, 4, 0xFF);
        *(uint16_t*)(&recv_buf[37])= Get_CRC16_Check_Sum(recv_buf, 37, 0xFFFF);
	write(ttyUSB, recv_buf, 39);
	//write(ttyUSB, &recv_buf[7], 30);
	cout<<"A Frame Sent!"<<endl;        
    }
}
